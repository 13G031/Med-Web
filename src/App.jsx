...
function Header() {
  ...
  const toggleLabel = language === 'zh' ? 'English' : '中文'
  ...
  return (
    <header className="site-header">
      ...
      <div className="nav-shell">
        <a className="brand" href="#top" aria-label={t('安帝源生物科技首页')}>
          <span>
            <strong>{t('安帝源生物科技')}</strong>
            <small>EMPO BIOTECH</small>
          </span>
        </a>

        <nav className="desktop-nav" aria-label={t('主要导航')}>
      ...
      >
        {navItems.map(([label, href]) => (
          <a key={href} href={href} onClick={() => setOpen(false)}>{t(label)}</a>
        ))}
        <a href="#contact" onClick={() => setOpen(false)}>{t('联系我们')}</a>
        <button
...
function ProofLedger() {
  ...
  return (
    <div className="proof-ledger" aria-label={t('核心资质与成果')}>
      {l(proofItems).map(({ icon: Icon, title, detail }) => (
...
function ProductArticle({ type, title, registration, image, imageWebp, imageAlt, imageWidth = 1448, imageHeight = 1086, imageFit, children, facts, details, interlude, reverse }) {
  const { l, t } = useLanguage()
  const localizedFacts = l(facts)
  const localizedDetails = l(details)
  return (
    <article className={`product-article ${interlude ? 'product-article-has-interlude' : ''} ${reverse ? 'product-article-reverse' : ''}`} data-reveal>
      ...
      <div className="product-copy">
        <div className="product-type">{t(type)}</div>
        <h3>{t(title)}</h3>
        <p>{t(children)}</p>
        <dl className="product-facts">
          ...
          {localizedFacts.map(([label, value]) => (
            <div key={label}>
              <dt>{label}</dt>
              <dd>{value}</dd>
            </div>
        ...
        <a className="text-link" href="#contact">
          {t('获取产品资料')} <ArrowRight size={17} aria-hidden="true" />
        </a>
      </div>
      {interlude && <div className="product-interlude">{interlude}</div>}
      <details className="product-details">
        <summary>{t('查看完整产品信息')} <CaretDown size={18} aria-hidden="true" /></summary>
        <div className="product-detail-grid">
          {localizedDetails.map(({ heading, intro, facts: detailFacts, codeGroups, codeRows, groups, procedures, items, ordered, timeline, note, wide }) => (
                ...
                <div className="product-code-groups">
                  {codeGroups.map(({ label, size, code }) => (
                    <div className="product-code-group" key={`${label}-${size}-${code}`}>
                      <h5>{label}</h5>
                      <p><span>{t('规格：')}</span>{size}</p>
                      <p className="product-code-value"><span>{t('编码：')}</span><strong>{code}</strong></p>
                    </div>
                  ))}
                </div>
              )}
    ...
    <form className="contact-form" onSubmit={submit} noValidate>
      ...
      {status === 'success' && (
        <p className="form-status form-success" role="status">
          {t('演示提交成功。接入正式后端后，留言才能发送给公司。')}
        </p>
      )}
      <button className="button" type="submit" disabled={status === 'loading'}>
        {t(status === 'loading' ? '正在提交' : '提交留言')}
...
function QuestionForm() {
  ...
  return (
    <form className="contact-form question-form" onSubmit={submit} noValidate>
      ...
      <p id="question-help" className="form-help">{t('问题内容至少需要 10 个字。')}</p>
      {status === 'error-email' && (
      ...
      {status === 'success' && (
        <p className="form-status form-success" role="status">
          {t('问题已记录。接入正式后端后，我们将通过电子邮箱回复您。')}
        </p>
      )}
      <button className="button" type="submit" disabled={status === 'loading'}>
        {t(status === 'loading' ? '正在提交' : '提交问题')}
        {status !== 'loading' && <ArrowRight size={18} aria-hidden="true" />}
      </button>
    </form>
  )
...
function EvidenceEntry({ item }) {
  const { l } = useLanguage()
  const localizedItem = l(item)
  return (
    <article>
      <div>
        <Buildings size={22} aria-hidden="true" />
        <strong>{localizedItem.institution}</strong>
      </div>
      <h4>{localizedItem.result}</h4>
      <dl className="clinical-detail">
        {localizedItem.details.map(([label, text]) => (
          <div key={label}>
            <dt>{label}</dt>
            <dd>{text}</dd>
          </div>
      ...
      <main id="main-content">
        <section id="top" className="hero-section">
          <div className="hero-grid">
            <div className="hero-copy">
              <h1>{t('快速止血的核心科技')}<br />{t('及应用展示')}</h1>
              <p className="hero-summary">{t('覆盖急救、手术与创面护理的双产品解决方案。')}</p>
              <div className="hero-actions">
                <a className="button" href="#products">
                  {t('查看产品')} <ArrowRight size={18} aria-hidden="true" />
                </a>
                <a className="button button-secondary" href="#evidence">{t('应用及展示')}</a>
              </div>
            </div>
            <figure className="hero-visual">
              <picture>
                ...
                <img
                  ...
                  alt={t('急救止血敷片、银色无菌内袋与白色外包装的产品视觉')}
                  width="1254"
                  height="1254"
                  fetchPriority="high"
                  decoding="async"
        ...
        <section id="about" className="section about-section">
          <div className="about-intro" data-reveal>
            <div className="about-copy">
              ...
              <div className="about-copy-body">
                <p>
                  {t('杨凌安帝源生物科技有限公司成立于2009年,位于陕西省杨凌示范区邰城南路10号，是一家集三类医疗器械研发、生产、销售为一体的综合性现代化高科技生物产业公司。')}
                </p>
                <p>
                  {t('公司拥有多项原创知识产权和生物科技独家技术。核心项目有“急救止血敷片”、“复方止血敷片”、“超薄生物止血膜”、“聚乙烯醇缩甲醛（PVF）医用海绵”、“HCV病毒复制” 等系列核心技术及产品。公司生物科技药械组合的“急救止血敷片”产品在院外紧急止血和院内急诊及手术中快速止血急救生命方面发挥重要作用。')}
                </p>
                <p>
                  {t('多年来公司一直坚持以100%的原创技术为经营发展方向，拥有多条国家级GMP认证的生产线和一支高质量生产及推广团队，核心原创技术产品均进入国家医保集采目录，在生物技术用于生命应急急救止血和急诊、手术中的快速止血领域具有一定的影响力。')}
                </p>
              </div>
            </div>
          ...
          <div className="about-purpose" data-reveal>
            <strong>{t('企业使命')}</strong>
            <p>
              {t('公司立足于生命急救产品的开发，用科技提升保护人类生命健康，提倡人们“关爱生命，珍惜生命。”用科技赋能打造高端医疗器械、智能数字化医疗器械、应急急救平台建设等，服务提升院前急救及急诊，手术临床水平，为“急救生命，呵护生命、健延生命”而奉献。')}
            </p>
          </div>
          ...
          <div className="about-rd" data-reveal>
            <div className="about-rd-lead">
              <div className="about-subheading">
                <h3>{t('核心科技/研发')}</h3>
                <p>{t('围绕急救止血敷片与聚乙烯醇甲醛PVF医用海绵形成原创知识产权、核心科技及应用场景。')}</p>
              </div>
              <figure className="about-rd-visual">
                <img
                  src="/images/cleanroom-production-v1.jpg"
                  alt={t('工作人员在洁净生产车间整理产品包装')}
                  width="1517"
                  height="1037"
                  loading="lazy"
                  decoding="async"
                />
                <figcaption>{t('洁净生产车间')}</figcaption>
              </figure>
            </div>
            <div className="about-rd-list">
              <article>
                <h4>{t('急救止血敷片')}</h4>
                <p>
                  {t('拥有原创知识产权和核心技术，创造性地将吸水海绵和多种生物活性药物组分相结合，采用独特的生产工艺使活性药物在常温下不失活性，通过调动人自身凝血因子快速止血，展示生物科技的魅力。')}
                </p>
              </article>
              <article>
                <h4>{t('独家地位')}</h4>
                <p>
                  {t('拥有核心科技，原创知识产权（发明专利号：ZL2005 10096239.4）全球唯一冠名为“急救止血敷片”的产品；国家医保集采（京津冀）招标以活性药物海绵独立分组（无同类同理医疗器械类同类比对产品）；也是我国唯一经过药、械联审的三类止血产品。')}
                </p>
              </article>
              <article>
                <h4>{t('聚乙烯醇缩甲醛（PVF）医用海绵')}</h4>
                <p>
                  {t('聚乙烯醇缩甲醛（PVF）医用海绵（ZL 2009 1 0021479.6）二类医疗器械（陕械注准20172140019）是拥有特殊技术的医疗新材料。')}
                </p>
              </article>
            </div>
          </div>
          ...
          <div className="about-records" data-reveal>
            <div className="about-subheading">
              <span className="product-type about-record-product-label">{t('急救止血敷片')}</span>
              <h3>{t('荣誉与公共服务')}</h3>
              <p>{t('按重大科技项目、国家部门及省级中标列配、灾害救援捐赠三个类别集中呈现。')}</p>
            </div>
            <div className="about-record-list">
              <details open>
                <summary>
                  <span>{t('重大科技项目荣誉')}</span>
                  <CaretDown size={20} aria-hidden="true" />
                </summary>
                <ul className="record-list">
                  {l(companyHonors).map((item) => <li key={item}>{item}</li>)}
                </ul>
              </details>
              <details>
                <summary>
                  <span>{t('国家部门及省级中标列配')}</span>
                  <CaretDown size={20} aria-hidden="true" />
                </summary>
                <ol className="record-list">
                  {l(institutionalDeployments).map((item) => <li key={item}>{item}</li>)}
                </ol>
              </details>
              <details>
                <summary>
                ...
                <ul className="record-list">
                  {l(reliefSupport).map((item) => <li key={item}>{item}</li>)}
                </ul>
              </details>
            </div>
          </div>
              ...
              <section className="mechanism-section" aria-labelledby="mechanism-title">
                <div className="section mechanism-inner">
                  <div className="section-heading section-heading-light">
                    <h2 id="mechanism-title">{t('三重止血功能，叠加互促，共同打开快速止血的凝血键')}</h2>
                    <p>{t('从调动凝血因子主动快速止血到吸液支撑，产品信息按临床用途和注册类别清晰分列。')}</p>
                  </div>
                  <ol className="mechanism-flow">
                    {l(mechanism).map((item, index) => (
                      <li key={item.title}>
                        <span>{String(index + 1).padStart(2, '0')}</span>
                        <div>
                          <h3>{item.title}</h3>
        ...
        <section id="evidence" className="section evidence-section">
          <div className="section-heading" data-reveal>
            <span className="product-type">{t('急救止血敷片')}</span>
            <h2>{t('动物实验  -  临床验证  -  产品上市后在临床中使用的再验征')}</h2>
          </div>
          ...
          <div className="evidence-layout">
            <div className="clinical-list" data-reveal>
              ...
              <aside className="clinical-summary" aria-labelledby="post-market-summary-title">
                <h2 id="post-market-summary-title">{t('产品上市后在临床中使用的再验征')}</h2>
                <dl>
                  <div>
                    <dt>{t('延续注册')}</dt>
                    <dd>{t('三类延续注册，已向NMPA提供上市后31家医院临床使用共232例不同创面的出血的止血样列（再次换证无需再提供临床反馈）。')}</dd>
                  </div>
                  <div>
                    <dt>{t('观察结果')}</dt>
                    <dd>{t('临床使用观察的效果得出：急救止血敷片232例观察中止血效果优及良好，无一例无效止血和不良反应。')}</dd>
                  </div>
                </dl>
              </aside>
              <a className="text-link" href="#contact">
                {t('咨询台（联系我们）')} <ArrowRight size={17} aria-hidden="true" />
              </a>
            </div>

            <div className="procurement-panel" data-reveal>
              <div className="procurement-heading">
                <h3>{t('国家医保医疗采购挂网与集采')}</h3>

              </div>
              <ol>
                {l(procurement).map(({ year, title, detail, scope, note }) => (
                  <li key={`${year}-${title}`}>
        ...
        <section className="section products-section products-section-final">
          <ProductArticle
            type="Ⅱ类医疗器械"
        ...
        <section id="support" className="section question-section">
          <div className="question-intro" data-reveal>
            <span className="product-type">{t('在线咨询')}</span>
            <h2>{t('问题与解答')}</h2>
            <p>{t('请留下您的电子邮箱和具体问题，我们将根据咨询内容准备回复。')}</p>
          </div>
          <div data-reveal>
            <QuestionForm />
          </div>
        ...
        <section id="contact" className="contact-section">
          <div className="section contact-layout contact-layout-centered" data-reveal>
            <div className="contact-copy">
              <h2>{t('需要产品资料或合作洽谈？')}</h2>
              <p>{t('请说明产品、临床科室或采购项目，我们将按您的需求准备资料。')}</p>
              <address>
                <a href="tel:02987071861"><Phone size={20} aria-hidden="true" />{t('029-87071861 / 186-2933-7891（中国） / 155-9618-5069 （国际）/')}</a>
                <a href="mailto:wilsion_yang@163.com"><EnvelopeSimple size={20} aria-hidden="true" />wilsion_yang@163.com</a>
                <span><MapPin size={20} aria-hidden="true" />{t('陕西省杨凌示范区邰城南路 10 号')}</span>
                <a href="https://www.empobiotech.com"><Clock size={20} aria-hidden="true" />{t('官网：www.empobiotech.com')}</a>
              </address>
            </div>
          </div>
        </section>
      ...
      <footer className="site-footer">
        <div>
          <a className="brand footer-brand" href="#top">
            <span><strong>{t('安帝源生物科技')}</strong><small>EMPO BIOTECH</small></span>
          </a>
          <p>{t('以原创生物材料技术服务急救、临床与生命健康。')}</p>
        </div>
        <div className="footer-links">
          <a href="#about">{t('关于我们')}</a>
          <a href="#products">{t('产品中心')}</a>
          <a href="#evidence">{t('临床与证据')}</a>
          <a href="#contact">{t('联系我们')}</a>
        </div>
        <div className="footer-meta">
          <span>{t('© 2026 杨凌安帝源生物科技有限公司')}</span>
          <span>{t('ICP备案号待补充')}</span>
        </div>
      </footer>
    </>
  )
...
export default function App() {
  ...
  const languageValue = useMemo(() => ({
    ...
    l: (value) => localize(value, language),
  }), [language])

  useEffect(() => {
    document.documentElement.lang = language === 'en' ? 'en' : 'zh-CN'
    document.title = language === 'en'
      ? 'Empo Biotech | Emergency Hemostasis Patch and PVF Medical Sponge'
      : '安帝源生物科技｜急救止血敷片与 PVF 医用海绵'
  }, [language])
